#!/usr/bin/env python3
"""Validate the Philippines v17 source package and result-free MUIO archive."""
...
REPO = PACKAGE.parent
CASE = REPO / "case" / "Philippines_v17"
ARCHIVE = PACKAGE / "muio" / "Philippines_v17_v17.0.0_MUIO.zip"
ARCHIVE_MANIFEST = PACKAGE / "data_sources" / "V17_MODEL_ARCHIVE_MANIFEST.csv"
LAND_VALIDATION = PACKAGE / "data_sources" / "snapshots" / "land_cover_validation.json"
PROVENANCE_VALIDATION = PACKAGE / "diagnostics" / "provenance_validation_v17.json"
OUTPUT = PACKAGE / "diagnostics" / "delivery_validation_v17.json"
...
def main() -> None:
    ...
    required = [
        ...
        PACKAGE / "data_sources" / "PHILIPPINES_V17_CANONICAL_SCHEMA_LEDGER.xlsx",
        ARCHIVE,
        LAND_VALIDATION,
    ...
    check("live_case_identity", gen["osy-casename"] == "Philippines_v17", gen["osy-casename"])
    check("land_validation", read_json(LAND_VALIDATION)["status"] == "pass", read_json(LAND_VALIDATION)["status"])
    ...
    with zipfile.ZipFile(ARCHIVE) as handle:
        ...
        for path in sorted(CASE.glob("*.json")):
            member = f"Philippines_v17/{path.name}"
    ...
    report = {
        "schema": "philippines-v17-delivery-validation-v1",
        "package": str(PACKAGE),
        "case": str(CASE),
        "archive": str(ARCHIVE),
