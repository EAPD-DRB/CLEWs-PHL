#!/usr/bin/env python3
"""Build and verify the result-free Philippines v20 portable MUIO archive."""
...
OUTPUT = PACKAGE / "muio" / "Philippines_v20_v20.0.0_MUIO.zip"
MANIFEST = PACKAGE / "data_sources" / "V20_MODEL_ARCHIVE_MANIFEST.csv"
CURRENT = PACKAGE / "data_sources" / "evidence" / "CURRENT_MODEL_ARCHIVE_MANIFEST.csv"
...
def main() -> None:
    ...
    with zipfile.ZipFile(OUTPUT, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for path in files:
            archive.write(path, Path("Philippines_v20") / path.relative_to(CASE))
    ...
    with MANIFEST.open("w", newline="", encoding="utf-8") as stream:
        ...
        writer.writerow([OUTPUT.name, archive_hash, OUTPUT.stat().st_size, len(files), "Philippines_v20/", "false",
                         "Complete editable v20.0.0 source and documentation; all res/ and generated solver/result files excluded."])

    with CURRENT.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
        writer.writerow(["package_relative_path", "size_bytes", "sha256", "zip_member_count", "internal_root"])
        writer.writerow([f"muio/{OUTPUT.name}", OUTPUT.stat().st_size, archive_hash, len(files), "Philippines_v20/"])
